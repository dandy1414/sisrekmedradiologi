...
<section class="content">
    <div class="row">
        <div class="col-xs-12">
            <form method="POST" action="{{ route('pasien.update-pasien-umum', ['id' => $pasien->id]) }}" enctype="multipart/form-data">
                @csrf
                {{ method_field('PUT') }}
                ...
                <div class="box box-success" style="position: relative;">
                    <div class="box-body">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group {{ $errors->first('jenisPasien') ? "has-error": "" }}">
                                    <label><span style="color: red">*</span> Jenis Pasien :</label>
                                    <div class="radio">
                                        <label>
                                            <input type="radio" name="jenisPasien" id="option-umum" value="umum"
                                                {{ $pasien->jenis_pasien == 'umum' ? "checked" : "" }} >
                                            Umum
                                        </label>
                                        <label>
                                            <input type="radio" name="jenisPasien" id="option-rs" value="rs"
                                                {{ $pasien->jenis_pasien == 'rs' ? "checked" : "" }}>
                                            Rumah Sakit
                                        </label>
                                ...
                                <div class="form-group {{ $errors->first('rekamMedis') ? "has-error": "" }}">
                                    ...
                                    <input value="{{ str_pad($pasien->nomor_rm, 6, '0', STR_PAD_LEFT) }}" type="text" name="rekamMedis" class="form-control"
                                ...
                                <div id="select-ruangan" class="form-group {{ $errors->first('asalRuangan') ? "has-error": "" }}" style="display: none">
...
<script type="text/javascript">
    $('#option-umum').click(function () {
        $('#select-ruangan').hide();
    })

    $('#option-rs').click(function () {
        $('#select-ruangan').show();
    })
